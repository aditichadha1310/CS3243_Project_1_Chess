...
def estimate_heuristic():
    ...
    for i in range(0, rows):
        for j in range(0, cols):
            heuristic_values[(chr(j + 97), i)] = 0
            for k in goal_position_list:
                k_c = k[0]
                k_r = int(k[1:])
                min_heuristic = max(abs((k_r - i)), abs((ord(k_c) - 97 - j)))
                if min_heuristic < heuristic_values[(chr(j+97), i)]:
                    heuristic_values[(chr(j + 97), i)] = min_heuristic
    ...
    for i in range(0, 8):
        ...
        if 0 <= (ord(ccc) - 97) < cols and 0 <= rr < rows:
            if visited.get((ccc, rr)) is False and (board_dictionary.get((ccc, rr)))[0] != 'X':
                total_path_cost_current = (board_dictionary.get((c, r)))[1] + heuristic_values[(ccc,rr)] + board_dictionary.get((ccc, rr))[1]
                heapq.heappush(queue, [total_path_cost_current, (ccc, rr), (c, r)])
...
def solve():
    ...
    queue = [((board_dictionary.get(start_pos))[1], start_pos, ())]
    heapq.heapify(queue)
    while queue:
        pos_tuple = heapq.heappop(queue)  # (cost, position tuple, parent tuple)
        tuple_pos = pos_tuple[1]  # position of tuple
        if visited[tuple_pos] is True:
            continue
        visited[tuple_pos] = True
        nodes_explored = nodes_explored + 1
        parent_child_mapping[tuple_pos] = pos_tuple[2]  # parent
        if start_pos is tuple_pos:
            board_dictionary.get(tuple_pos)[1] =0
        else:
            # update path cost
            board_dictionary.get(tuple_pos)[1] += board_dictionary.get(pos_tuple[2])[1]
        if board_dictionary.get((tuple_pos[0], tuple_pos[1]))[0] == 'G':  # is goal state
            goal_pos_reached.append(tuple_pos[0])
            goal_pos_reached.append(tuple_pos[1])
            total_path_cost = board_dictionary.get(tuple_pos)[1]
            return goal_pos_reached
        explore_neighbours(tuple_pos[1], tuple_pos[0])
    return 0
...
def run_AStar():
    ...
    if goal_attained:
        ...
        if len(path) != 0:
            path.reverse()
            return (path, nodes_explored, total_path_cost)
    else:
        return ([], 0, 0)
